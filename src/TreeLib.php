...
class TreeLib
{
    ...
    {
        if (empty($data)) {
            return [];//throw new \Exception('请先调用load入参data');
        }
        $root = $root === null ? 0 : $root;
        self::$_pkField = $options['pk_field'] ?? 'id';
        self::$_parentField = $options['parent_field'] ?? 'parent_id';
        self::$_childrenField = $options['children_field'] ?? 'children';
